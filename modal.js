...
productosElementos.forEach((productosElementos) => {
  productosElementos.addEventListener("click", (e) => {
    ...
    if (productosElementos) {
      const productoId = productosElementos.id;
...
function realizarCompra() {
  ...
  if (carroCompras.length === 0) {
    modalContenido.innerHTML = `
      <img src="Imagenes/error.png" class="modal_img" />
      <h2 class="modal_title">¡Ups!</h2>
      <p class="modal_description"><center>Tu carrito de compras está vacío, pero tu estómago no debería estar igual. Haz tu pedido y vuelve aquí para pagar.</center></p>
      <a href="#" class="modal_close">Cerrar</a>
    `;
  } else {
    const tiempoAproximado = Math.floor(Math.random() * (30 - 10 + 1)) + 10;

    modalContenido.innerHTML = `
      <img src="Imagenes/logo_Modal.png" class="modal_img" />
      <h2 class="modal_title">Compra Exitosa</h2>
      <p>Tiempo Aproximado: ${tiempoAproximado} minutos</p>
      <p class="modal_description"><center>¡Gracias por elegir BurgerClassic! 🍔💙 Tu satisfacción es nuestra mayor recompensa. Esperamos verte de nuevo pronto. ¡Buen provecho y regresa cuando quieras!</center></p>
      <a href="#" class="modal_close">Cerrar</a>
    `;
